...
function ClientesPage() {
  ...
  const load = async () => {
    ...
    setRows(data || []);
    const { data: a } = await supabase.from("arquitetos").select("id,nome").eq("status", true).order("nome");
    setArqs(a || []);
  };
  useEffect(() => { load(); }, []);
    ...
    <div className="p-4 md:p-8 max-w-[1400px] mx-auto">
      ...
      <Card className="p-4 mb-4">
        <div className="relative">
          ...
          <Input className="pl-9" placeholder="Buscar por nome, CPF/CNPJ, e-mail, número..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      </Card>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="md:col-span-2"><Label>Informações adicionais</Label><Textarea rows={3} value={form.informacoes_adicionais} onChange={(e) => setForm({ ...form, informacoes_adicionais: e.target.value })} /></div>
          </div>
          <DialogFooter>
